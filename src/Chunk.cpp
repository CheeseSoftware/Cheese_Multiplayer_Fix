...
#ifndef _SERVER
void Chunk::Draw(long xPos, long yPos, App &app, GameUtilitiyInterface *gameUtilitiyInterface)
{
	for(long x = 0; x < CHUNKWIDTH; x++)
	{
		for(long y = 0; y < CHUNKHEIGHT; y++)
		{
			...
			for(int l = 5; l >= 0; l--)
			{
				...
				{
					blockList[x][y][l].first->Draw((xPos - 1 << 4) + x << 4, (yPos - 1 << 4) + y << 4, app, gameUtilitiyInterface, blockList[x][y][l].second); 
